...
# Optional for future automatic inference:
# model_directory = save_model(
#     model=model,
#     model_name="efficientnet_b0_image_h1",
#     model_type=2,
#     inference_metadata={
...
defined in configs/data_config.yaml.
...
The optional inference_metadata argument can store the information needed to
reconstruct inference later. Existing notebooks can omit it.
...
Prediction cache
----------------
Every successful call to evaluate() also saves:

    validation_predictions.parquet

inside the model directory. This allows final-model selection and multimodal
fusion to reuse validation probabilities without retraining the base model.
...
def save_model(
    ...
    *,
    ...
    ----------
    inference_metadata:
        Optional JSON-compatible dictionary describing the information needed
        to reconstruct inference later. Existing notebooks may omit it.
    """
    ...
    if inference_metadata is not None and not isinstance(
        inference_metadata,
        dict,
    ):
        raise TypeError("inference_metadata must be a dict or None.")
...
def _validate_and_merge_split(
    ...
    """Validate and merge predictions with one data split."""
...
def select_clinical_threshold(
    ...
    """Select threshold maximizing specificity at target sensitivity."""
...
def save_evaluation_figures(
    ...
    """Create and save train/validation evaluation figures."""
    return {
...
def save_prediction_cache(
    ...
    *,
    ...
    """Validate and save canonical lesion-level probability predictions."""
    ...
    missing_columns = required_columns - set(df_predictions.columns)

    if missing_columns:
        raise KeyError(
            f"{split_name} predictions are missing columns: {sorted(missing_columns)}"
        )
    ...
    if predictions["isic_id"].isna().any():
        raise ValueError(
            f"{split_name} predictions contain missing lesion identifiers."
        )

    if predictions["isic_id"].duplicated().any():
        raise ValueError(
            f"{split_name} predictions contain duplicated lesion identifiers."
        )

    predictions["isic_id"] = predictions["isic_id"].astype(str)
    ...
    if predictions["probability"].isna().any():
        raise ValueError(f"{split_name} predictions contain missing probabilities.")

    if not np.isfinite(predictions["probability"]).all():
        raise ValueError(f"{split_name} predictions contain non-finite probabilities.")

    if (
        not predictions["probability"]
        .between(
            0.0,
            1.0,
        )
        ...
        raise ValueError(f"{split_name} probabilities must be between 0 and 1.")
    ...
    if not model_directory.is_dir():
        raise NotADirectoryError(
            f"model_directory must be a directory: {model_directory}"
        )

    output_path = model_directory / f"{split_name}_predictions.parquet"
...
def save_evaluation_metadata(
    ...
    prediction_cache_paths: dict[str, Path],
    run_directory: Path,
) -> Path:
    ...
    metadata = {
        ...
        "outputs": {
            ...
            "prediction_cache_files": (prediction_cache_paths),
        },
    }
...
def evaluate(
    ...
    *,
    ...
    The clinical threshold is selected exclusively from validation and then
    applied unchanged to train and validation.

    Validation probabilities are also cached as
    validation_predictions.parquet inside the model directory.
    """
    ...
    prediction_cache_paths = {
        "validation": save_prediction_cache(
            df_predictions=df_validation_predictions,
    ...
    evaluation_metadata_path = save_evaluation_metadata(
        ...
        prediction_cache_paths=(prediction_cache_paths),
        run_directory=run_directory,
    )
